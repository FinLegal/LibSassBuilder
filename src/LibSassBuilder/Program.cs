...
namespace LibSassBuilder
{
    class Program
    {
                    ...
                    switch (o)
                    {
                        case DirectoryOptions directory:

                            {
                                ...
                                await program.CompileDirectoriesAsync(
                                    directory.Directory,
        ...
        async Task CompileDirectoriesAsync(
            string directory,
            IEnumerable<string> excludedDirectories
        )
        {
            var sassFiles = Directory
            ...
            foreach (var subDirectory in subDirectories)
            {
                ...
                if (
                    excludedDirectories.Any(
                        dir => string.Equals(dir, directoryName, StringComparison.OrdinalIgnoreCase)
                    )
                )
                    continue;
        ...
        async Task CompileFilesAsync(IEnumerable<string> sassFiles)
        {
            foreach (var file in sassFiles)
            {
                ...
                var result = SassCompiler.CompileFile(
                    file,
                    options: Options.SassCompilationOptions
                );
                ...
                if (
                    File.Exists(newFile)
                    && result.CompiledContent.ReplaceLineEndings()
                        == (await File.ReadAllTextAsync(newFile)).ReplaceLineEndings()
                )
                    continue;
